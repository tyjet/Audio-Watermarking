...
void embedWatermarK(int** inputStream, char* watermarK, int watermarKLength, int** &outputStream, int* &secretKey)
{
  ...
  */
  ...
  int coefficientF, coefficientR, inputStreamF, inputStreamR;
  
  /* Generate the DCT coefficients for each segment */
  for (int segmentIdx = 0; segmentIdx < NUM_SEGMETNS; ++segmentIdx)
  {
    coefficientF = 0.0;
    coefficientR = 0.0;
    for (int coefficientIdx = 0; coefficientIdx < L_OVER_TWO; ++sampleIdx)
    {
      for (int sampleIdx = 0; sampleIdx < L_OVER_TWO; ++idx)
      {            
        inputStreamF = inputStream[segmentIdx][sampleIdx];
        inputStreamR = inputStream[segmentIdx][sampleIdx + L_OVER_TWO];
        
        dctMultiplier = (sampleIdx) ? 2 * ONE_OVER_SQRT_L : SQRT_TWO * ONE_OVER_SQRT_L;
  ...
  {
    for (int coefficientIdx = 0; coefficientIdx < NUM_COEFFICIENTS; ++idx)
    {
  ...
  */
  ...
  double pVal, p1Val, p2Val, pTIldeVale, qVal, q1Val, q2Val, qTildeVal;
  ...
  */
  ...
  for (int segmentIdx = 0; segmentIdx < NUM_SEGMENTS && curWatermarKBit < watermarKLength; ++segmentIdx)
  {
    ...
    for (int frameIdx = 0; frameIdx < R; ++frameIdx)
    {
      if (useFrame[segmentIdx][frameIdx])
      {
        ...
        pPrime[segmentIdx][frameIdx] = 0.5 * (p1Prime + p2Prime);
        qPrime[segmentIdx][frameIdx] = 0.5 * (q1Prime + q2Prime);
  ...
  double idctMultiplier, outputStreamF, outputStreamR;
  for (int segmentIdx = 0; segmentIdx < NUM_SEGMENTS; ++segmentIdx)
  {
    for (int sampleIdx = 0; sampleIdx < L_OVER_TWO; ++sampleIdx)
    {
      ...
      {
        idctMultiplier = (coefficientIdx) ? 4 * ONE_OVER_SQRT_L : SQRT_TWO * ONE_OVER_SQRT_L;
